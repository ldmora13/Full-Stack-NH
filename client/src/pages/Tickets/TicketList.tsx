...
function CreateButton({ variant = 'primary' }: { variant?: 'primary' | 'secondary' | 'outline' | 'danger' }) {
    ...
    return (
        <Link to="/tickets/new">
            <Button variant={variant} className={variant === 'primary' ? "shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/30 transition-all" : "w-auto"}>
        ...
        <Layout>
            ...
            {isLoading ? (
                ...
                <motion.div
                    ...
                    className="text-center py-24 bg-slate-900/30 rounded-3xl border border-dashed border-slate-800"
                >
                                ...
                                return (
                                    ...
                                    >
                                        <Link to={`/ tickets / ${ticket.id} `}>
                                            <div className="group glass p-5 rounded-2xl hover:bg-slate-800/60 hover:border-emerald-500/30 transition-all cursor-pointer relative overflow-hidden">
                                                ...
                                                <div className="relative z-10 flex items-start justify-between gap-4">
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        <div className="flex items-center gap-2 mb-3">
                                                            <div className="flex items-center gap-1.5 text-xs font-medium text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">
                                                                {(() => {
                                                    ...
                                                    <div className="flex flex-col items-end gap-2">
                                                        ...
                                                        <span className={`px - 2.5 py - 0.5 rounded - full text - xs font - medium border border - transparent ${priorityStyle.bg || 'bg-slate-800'} ${priorityStyle.color} `}>
                                                            {t(`ticket_priority.${ticket.priority} `)}
                                                        </span>
                                                    </div>
                                                </div>
                                            </div>
